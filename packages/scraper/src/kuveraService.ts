...
export class KuveraService {
  private readonly listApiUrl = 'https://api.kuvera.in/mf/api/v4/fund_schemes/list.json';
  ...
  async fetchFilteredFundCodes(): Promise<KuveraFund[]> {
    ...
    try {
      const response = await axios.get(this.listApiUrl, { headers: this.headers });
            ...
            for (const fund of directGrowthFunds) {
              ...
              if (fund.n.toUpperCase().includes('DIRECT')) {
                filteredFunds.push({
                  ...
                  category,
                  fundHouse,
                  nav: typeof fund.v === 'number' ? fund.v : parseFloat(fund.v || '0'),
      ...
      console.log(`Found ${filteredFunds.length} valid Direct Growth funds passing the Atlas Filter.`);
