...
export class AmfiService {
  ...
  private parseAmfiData(rawData: string): NAVRecord[] {
    ...
    for (const line of lines) {
      ...
      if (
        !trimmedInfo ||
        trimmedInfo.includes("Open Ended Schemes") ||
        trimmedInfo.includes("Close Ended Schemes") ||
        !trimmedInfo.includes(";")
      ) {
        continue;
      }
      ...
      const [schemeCode, isin1, isin2, schemeName, navStr, date] =
        trimmedInfo.split(";");
